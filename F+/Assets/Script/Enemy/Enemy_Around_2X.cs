...
public class Enemy_Around_2X : EnemyBase
{
    // 索敵範囲
    [SerializeField]
    ...
    protected override bool Move()
    {
        ...
        if (MapData.instance.GetMapChipType(targetPoint) == MapData.MapChipType.room)
        {
            ...
            {// 部屋の中かつ、プレイヤーと同じ部屋
             // 一回分探索した後の座標を取得  // 8方向
                this.status.movedPoint = this.aStar.A_StarProc_Specified(2,true);
            }
            else
            {// 経路探索を簡単なやつにする
                this.status.movedPoint = this.aStar.SimpleProc(status.point);
                this.status.movedPoint = this.aStar.SimpleProc(this.status.movedPoint);
            }
        }
        else
        {// 経路探索を簡単なやつにする
