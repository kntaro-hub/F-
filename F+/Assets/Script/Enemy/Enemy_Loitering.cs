...
public class Enemy_Loitering : EnemyBase
{
    ...
    // 動作タイプ
    private TargetType targetType = TargetType.loitering;
    ...
    // 
    private bool isLoiteringTarget = false;
    ...
    void Start()
    {
        ...
        aStar.SetStartPoint(status.point);
    ...
    protected override bool Move()
    {
        ...
        {
            // 徘徊中
            case TargetType.loitering:
                    if(this.status.point == TargetPoint)
                    {
                        // ターゲットに到着している場合
                        isLoiteringTarget = false;
                    }

                    if(!isLoiteringTarget)
                    {
                        this.SetTargetRoom();
                    }
                break;
    ...
    {
        // ランダムな部屋の中心をターゲットにする
        Division.Div_Room room = rooms[Random.Range(0, rooms.Count)].Room;
        TargetPoint = room.Center();
        isLoiteringTarget = true;
    }
